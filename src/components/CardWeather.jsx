...
export default function CardWeather() {
    ...
    return (
        <div className="bg-colorCard1 md:row-span-2 lg:row-span-2 p-4 rounded-3xl flex flex-row gap-6 justify-between items-center shadow-md">

            <div className="w-full">
                <p className="font-custom1 text-3xl mt-4 text-colorDark">
                    {description ? description.toUpperCase() : "Unknown weather"}
                </p>

                <p className="font-custom2 text-xl text-colorMedium">
                    {clouds ? `Cloud coverage ${clouds}%` : ""}
                </p>

                <p className="font-custom1 text-3xl flex gap-1 items-center justify-center mt-6 bg-colorDark text-colorBrand rounded-xl p-2">
                    <Thermometer size={24}/>{temp ? temp.toFixed(0) : "??"}°C
                </p>

                <p className="font-custom2 text-2xl flex justify-center mt-2 text-colorMedium">
                    Feels like {feels ? feels.toFixed(0) : "??"}°C
                </p>

                <p className="font-custom1 text-2xl flex gap-1 items-center justify-center mt-4 text-colorDark">
            ...
            <div>
                <svg width="30" height="400" xmlns="http://www.w3.org/2000/svg" transform="rotate(180)">
